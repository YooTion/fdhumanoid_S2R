...
class HumaRobotSimRos(Node):
    ...
    def get_obs(self, data):
        ...
        # print("torso_quat:", quaternion_to_euler_array(torso_quat))
        # print("torsop_quat:", quaternion_to_euler_array(torsop_quat))
        # print("torso_omega:", torso_omega)
        # print("torsop_omega:", torsop_omega)
        ...
        return (q, dq, quat, v, omega, gvec, lin_acc, torque_foot, lin_vel, force_foot, src_force_foot, torso_quat, torso_omega)
    ...
    def run_mujoco(self, policy, param, cfg):
        ...
        with Timer(f'[INFO]: Time taken for {cfg.sim_config.sim_duration:.2f}s simulation\r'):
            for _ in range(int(cfg.sim_config.sim_duration / cfg.sim_config.dt)):
                ...
                q, dq, quat, v, omega, gvec, lin_acc, torque_foot, lin_vel, force_foot, src_frc_foot, torso_quat, torso_omega = self.get_obs(data)
                ...
                if count_lowlevel % cfg.sim_config.decimation == 0:
                    target_q = self.rl_policy_forward(param=param, policy=policy, q=q, dq=dq, waist_orient_quat=quat,
                                                      waist_omega=omega, torque_foot=self.tau, force_foot=force_foot,
                                                      lin_vel=lin_vel, torso_orient_quat=torso_quat, torso_omega=torso_omega)
                # 低级控制
    ...
    def rl_policy_forward(self, param, policy, q, dq, waist_orient_quat, waist_omega, torque_foot, force_foot, lin_vel, torso_orient_quat, torso_omega):
        """."""
        ...
        # 扭矩观测
        self.obs_torques[4] = torque_foot[4]
        self.obs_torques[5] = torque_foot[5]
        self.obs_torques[10] = torque_foot[10]
        self.obs_torques[11] = torque_foot[11]
        # 欧拉角
        eu_ang = quaternion_to_euler_array(waist_orient_quat)
        eu_ang[eu_ang > math.pi] -= 2 * math.pi

        torso_eu_ang = quaternion_to_euler_array(torso_orient_quat)
        torso_eu_ang[torso_eu_ang > math.pi] -= 2 * math.pi
        ...
        if param['has_obs_torque']:
            obs[0, param['obs_index_torque'] : param['obs_index_torque'] + len(self.obs_torques)] =\
                self.obs_torques * self.obs_torques_mark * param['obs_scales_torque']
        ...
        if param['has_obs_imu_torso']:
            obs[0, param['obs_index_imu_torso_angvel'] : param['obs_index_imu_torso_angvel'] + len(torso_omega)] = \
                torso_omega * param['obs_scales_imu_torso_angvel']
            obs[0, param['obs_index_imu_torso_orientation'] : param['obs_index_imu_torso_orientation'] + len(torso_eu_ang)] = \
                torso_eu_ang * param['obs_scales_imu_torso_orientation']
